...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (email && env.RESEND_API_KEY) {
      ...
      const html = `
        <div style="font-family:monospace;font-size:13px;color:#333;max-width:600px;margin:0 auto;padding:32px;">
          ...
          ${messages.map((m) => `
            <div style="margin-bottom:20px;${m.role === 'user' ? 'text-align:right;' : ''}">
              <span style="font-size:9px;letter-spacing:.1em;color:#bbb;text-transform:uppercase;display:block;margin-bottom:4px;">
                ${m.role === 'user' ? 'YOU' : 'EFFECT AI'}
              </span>
      ...
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'EFFECT AI <noreply@effect.moe>',
          to: [email],
